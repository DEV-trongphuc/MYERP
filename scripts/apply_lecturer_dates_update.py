import json
import subprocess
import sys
import re

sys.stdout.reconfigure(encoding='utf-8')

SSH_CMD = [
    'ssh', '-i', r'C:\Users\LENOVO\.ssh\id_ed25519',
    '-4', '-p', '2210',
    '-o', 'StrictHostKeyChecking=no',
    'vhvxoigh@chiefaiofficer.vn'
]

def run_remote_sql(sql):
    proc = subprocess.Popen(
        SSH_CMD + ["mysql -u vhvxoigh_mail_auto -p'Ideas@812' vhvxoigh_myerp --default-character-set=utf8mb4"],
        stdin=subprocess.PIPE,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
        encoding='utf-8'
    )
    out, err = proc.communicate(input=sql)
    return out, err

with open('scratch/lecturer_due_dates.json', 'r', encoding='utf-8') as f:
    due_dates = json.load(f)

# Load DB expenses
sql = "SELECT id, title, date FROM expenses WHERE deleted_at IS NULL;"
out, _ = run_remote_sql(sql)

db_map = {}
for l in out.strip().split('\n')[1:]:
    p = l.split('\t')
    if len(p) >= 3:
        eid, title, dt = int(p[0]), p[1], p[2]
        m = re.search(r'\[MISA #(\d+)\]', title)
        if m:
            mid = m.group(1)
            db_map[mid] = (eid, title, dt)

sql_updates = []
count = 0
for mid, info in due_dates.items():
    if str(mid) in db_map:
        eid, title, cur_dt = db_map[str(mid)]
        new_dt = info['due_date']
        if cur_dt != new_dt:
            sql_updates.append(f"UPDATE expenses SET date = '{new_dt}' WHERE id = {eid};")
            count += 1
            print(f"Updating ID {eid} (MISA #{mid}): {cur_dt} -> {new_dt} ({info['type']})")

if sql_updates:
    combined_sql = "\n".join(sql_updates)
    print(f"\nExecuting {len(sql_updates)} updates on live database...")
    out, err = run_remote_sql(combined_sql)
    if err:
        print("Error:", err)
    else:
        print("Live Database updated successfully!")
else:
    print("No updates needed.")
