...
const ALLOWED_PORTAL_ROLES = ['sale', 'sales', 'superadmin', 'admin', 'super_admin', 'manager', 'director', 'assistant', 'viewer', 'hr', 'accountant', 'marketing', 'academic', 'sale_admin', 'saleadmin', 'staff', 'employee', 'telesale', 'prescreener'];
...
{/* Middle section: Issues/Tasks */}
  ...
  const renderScheduleView = () => {
    ...
    const canEditUserAssets = ['admin', 'superadmin', 'super_admin', 'director', 'manager', 'hr'].includes(String(user?.role).toLowerCase());

    const handleManagerOnlyFieldClick = () => {
      if (!canEditUserAssets) {
        toast(t('Chỉ có Quản lý / Ban Giám đốc / HR mới có quyền cập nhật trường này.'), { icon: 'ℹ️' });
      }
    };
                    ...
                    <div className="form-group" style={{ marginTop: '0.5rem', paddingTop: '1rem', borderTop: '1px solid var(--color-border-light)' }}>
                      ...
                      {saleSignatureUrl ? (
                        <div style={{
                          ...
                          <img 
                            src={saleSignatureUrl ? (saleSignatureUrl.startsWith('http') || saleSignatureUrl.startsWith('data:') ? saleSignatureUrl : `https://myerp.ideas.edu.vn/backend/${saleSignatureUrl.replace(/^\/+/, '')}`) : ''} 
                            alt="Chữ ký mẫu" 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? '0.75rem' : '1rem' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? '0.75rem' : '1rem' }}>
                      <div className="form-group" onClick={handleManagerOnlyFieldClick} style={{ cursor: !canEditUserAssets ? 'pointer' : 'default' }}>
                        <label className="form-label" style={{ fontWeight: 600, fontSize: isMobile ? '0.75rem' : '0.875rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? '0.75rem' : '1rem' }}>
                      ...
                      <div className="form-group" onClick={handleManagerOnlyFieldClick} style={{ cursor: !canEditUserAssets ? 'pointer' : 'default' }}>
                        <label className="form-label" style={{ fontWeight: 600, fontSize: isMobile ? '0.75rem' : '0.875rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <span>{t('Loại hợp đồng')}</span>
                          {!canEditUserAssets && <span style={{ fontSize: '0.7rem', color: 'var(--color-text-muted)', fontWeight: 400 }}>🔒 {t('Chỉ Quản lý / HR')}</span>}
                        </label>
                        <div style={{ pointerEvents: !canEditUserAssets ? 'none' : 'auto' }}>
                          <CustomSelect
                            options={[
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? '0.75rem' : '1rem' }}>
                      ...
                      <div className="form-group" onClick={handleManagerOnlyFieldClick} style={{ cursor: !canEditUserAssets ? 'pointer' : 'default' }}>
                        <label className="form-label" style={{ fontWeight: 600, fontSize: isMobile ? '0.75rem' : '0.875rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <span>{t('Người quản lý trực tiếp')}</span>
                          {!canEditUserAssets && <span style={{ fontSize: '0.7rem', color: 'var(--color-text-muted)', fontWeight: 400 }}>🔒 {t('Chỉ Quản lý / HR')}</span>}
                        </label>
                        <div style={{ pointerEvents: !canEditUserAssets ? 'none' : 'auto' }}>
                          <CustomSelect
              ...
              <div className={isMobile ? "animate-fade-in" : "card animate-fade-in"} style={cardContainerStyle(isMobile, '20px')}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '12px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <button
                      type="button"
                      className={isEditingCertificates ? "btn primary" : "btn outline"}
                ...
                {!isEditingCertificates ? (
                  ...
                  <>
                    {profileCertificates.length === 0 ? (
                      <div style={{ textAlign: 'center', padding: '2rem', background: 'var(--color-bg)', borderRadius: '12px', border: '1px dashed var(--color-border-light)', marginBottom: '1rem' }}>
                        <span style={{ fontSize: '0.825rem', color: 'var(--color-text-muted)', fontStyle: 'italic' }}>
                          {t('Chưa có chứng chỉ hoặc bằng cấp nào được thêm.')}
                        </span>
                      </div>
                    ...
                    <button
                      ...
                      style={{ width: 'fit-content', alignSelf: 'flex-start', marginTop: '1rem', display: 'flex', alignItems: 'center', gap: '6px' }}
                    >
                      <Plus size={16} />
              ...
              <div className={isMobile ? "animate-fade-in" : "card animate-fade-in"} style={cardContainerStyle(isMobile, '20px')}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '12px' }}>
                  ...
                  {['admin', 'superadmin', 'manager', 'assistant'].includes(String(user?.role).toLowerCase()) && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <button
                        type="button"
                        className={isEditingHRRecords ? "btn primary" : "btn outline"}
                ...
                <div style={{ 
                  ...
                  >
                    ...
                    <span style={{ 
                      ...
                      {profileHRRecords.filter(r => r.type === 'award').length}
                    </span>
                  </button>
                  ...
                  >
                    ...
                    <span style={{ 
                      ...
                      {profileHRRecords.filter(r => r.type === 'warning').length}
                    </span>
                  </button>
                  ...
                  >
                    ...
                    <span style={{ 
                      ...
                      {profileHRRecords.filter(r => r.type === 'discipline').length}
                    </span>
                  </button>
                </div>
                ...
                {!isEditingHRRecords ? (
                  ...
                  profileHRRecords.filter(r => r.type === activeHRSubTab).length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                      {profileHRRecords.filter(r => r.type === activeHRSubTab).map((record, index) => {
                        ...
                        return (
                          <div key={record.id || index} style={{
                            ...
                            gap: '0.75rem',
                            padding: '1.25rem 0',
                            borderBottom: index < profileHRRecords.filter(r => r.type === activeHRSubTab).length - 1 ? '1px solid var(--color-border-light)' : 'none',
                            position: 'relative'
                          }}>
                  ...
                  <>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                      {profileHRRecords.filter(r => r.type === activeHRSubTab).map((record) => {
                        const originalIndex = profileHRRecords.findIndex(r => r.id === record.id);
                    ...
                    <button
                      ...
                      style={{ width: 'fit-content', alignSelf: 'flex-start', marginTop: '1rem', display: 'flex', alignItems: 'center', gap: 6 }}
                    >
                      <Plus size={16} />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                  ...
                  <form onSubmit={handleChangePasswordSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    ...
                    {(() => {
                      ...
                      return (
                        <>
                          <div style={{ background: 'rgba(59, 130, 246, 0.05)', border: '1px solid rgba(59, 130, 246, 0.2)', padding: '10px 14px', borderRadius: '8px' }}>
                            <p style={{ color: 'var(--color-text)', fontSize: '0.8rem', margin: '0 0 6px 0', fontWeight: 600 }}>
                              {t('Quy định mật khẩu: Tối thiểu 8 ký tự, bao gồm cả chữ cái và chữ số')}
                            </p>
                            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', fontSize: '0.72rem' }}>
                              <span style={{ color: isLongEnough ? '#059669' : '#64748b', display: 'inline-flex', alignItems: 'center', gap: '4px', fontWeight: 600, background: isLongEnough ? 'rgba(16, 185, 129, 0.1)' : 'rgba(0,0,0,0.04)', padding: '2px 8px', borderRadius: '4px' }}>
                                {isLongEnough ? <Check size={12} color="#059669" /> : '○'} {t('Tối thiểu 8 ký tự')}
                              </span>
                              <span style={{ color: hasLetter ? '#059669' : '#64748b', display: 'inline-flex', alignItems: 'center', gap: '4px', fontWeight: 600, background: hasLetter ? 'rgba(16, 185, 129, 0.1)' : 'rgba(0,0,0,0.04)', padding: '2px 8px', borderRadius: '4px' }}>
                                {hasLetter ? <Check size={12} color="#059669" /> : '○'} {t('Chứa chữ cái (a-z, A-Z)')}
                              </span>
                              <span style={{ color: hasDigit ? '#059669' : '#64748b', display: 'inline-flex', alignItems: 'center', gap: '4px', fontWeight: 600, background: hasDigit ? 'rgba(16, 185, 129, 0.1)' : 'rgba(0,0,0,0.04)', padding: '2px 8px', borderRadius: '4px' }}>
                                {hasDigit ? <Check size={12} color="#059669" /> : '○'} {t('Chứa chữ số (0-9)')}
                              </span>
                            </div>
                          </div>
                            ...
                            <div className="form-group" style={{ margin: 0 }}>
                              ...
                              {isConfirmTouched && (
                                <div style={{
                                  ...
                                  {isConfirmMatch ? (
                                    <><Check size={13} color="#059669" /> {t('Mật khẩu xác nhận trùng khớp')}</>
                                  ) : (
                                    <><X size={13} color="#dc2626" /> {t('Mật khẩu xác nhận không khớp')}</>
                                  )}
                                </div>
                              )}
                            </div>
                          ...
                          <button
                            ...
                            disabled={changingPass || !oldPass || !isStrong || !isConfirmMatch}
                  ...
                  {todayCheckIn ? (
                    <div
                      style={{
                        ...
                        fontSize: '0.7rem',
                        fontWeight: 600,
                        cursor: (todayCheckIn.status === 'rejected' || (requireCheckout && !todayCheckIn.check_out_time)) ? 'pointer' : 'default',
                        border: '1px solid',
                        backgroundColor: 
                          todayCheckIn.check_out_time ? 'rgba(59, 130, 246, 0.12)' :
                          todayCheckIn.status === 'approved' ? 'rgba(255, 255, 255, 0.12)' :
                          todayCheckIn.status === 'pending_approval' ? 'rgba(245, 158, 11, 0.18)' :
                          'rgba(239, 68, 68, 0.18)',
                        color: 
                          todayCheckIn.check_out_time ? '#93c5fd' :
                          todayCheckIn.status === 'approved' ? '#ffffff' :
                          todayCheckIn.status === 'pending_approval' ? '#ffe066' :
                          '#ff8888',
                        borderColor: 
                          todayCheckIn.check_out_time ? 'rgba(59, 130, 246, 0.25)' :
                      ...
                      title={
                        todayCheckIn.check_out_time ? t('Đã hoàn thành ngày làm việc') :
                        todayCheckIn.status === 'approved' ? (requireCheckout ? t('Click để Chấm công Ra ca') : t('Đã chấm công thành công')) :
                    ...
                    >
                      <span style={{
                        ...
                        borderRadius: '50%',
                        backgroundColor: 
                          todayCheckIn.check_out_time ? '#3b82f6' :
                      ...
                      {todayCheckIn.status === 'approved' && (
                        todayCheckIn.check_out_time
                          ? `${t('Đã Ra ca')} (${todayCheckIn.check_out_time.substring(11, 16) || todayCheckIn.check_out_time.substring(0, 5)})`
                          : `${t('Đã Check-in')} (${todayCheckIn.check_in_time.substring(0, 5)})`
                      )}
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--color-border-light)', marginBottom: '1.25rem', gap: '1.5rem' }}>
              <button
                ...
                style={{
                  ...
                  fontSize: '0.875rem',
                  fontWeight: 700,
                  color: schedulerModalTab === 'diary' ? 'var(--color-primary)' : 'var(--color-text-light)',
                  border: 'none',
                  background: 'transparent',
                  borderBottom: schedulerModalTab === 'diary' ? '2px solid var(--color-primary)' : '2px solid transparent',
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  display: 'flex',
                  alignItems: 'center',
              ...
              >
                ...
                <span style={{
                  ...
                  padding: '2px 6px',
                  borderRadius: '10px',
                  background: schedulerModalTab === 'diary' ? 'var(--color-primary-light)' : 'var(--color-bg)',
              ...
              <button
                type="button"
                onClick={() => setSchedulerModalTab('tasks')}
                style={{
                  padding: '8px 4px 12px 4px',
                  fontSize: '0.875rem',
                  fontWeight: 700,
                  color: schedulerModalTab === 'tasks' ? 'var(--color-primary)' : 'var(--color-text-light)',
                  border: 'none',
                  background: 'transparent',
                  borderBottom: schedulerModalTab === 'tasks' ? '2px solid var(--color-primary)' : '2px solid transparent',
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  display: 'flex',
                  alignItems: 'center',
              ...
              >
                ...
                <span style={{
                  ...
                  padding: '2px 6px',
                  borderRadius: '10px',
                  background: schedulerModalTab === 'tasks' ? 'var(--color-primary-light)' : 'var(--color-bg)',
                        ...
                        if (dayNotes.length === 0) {
                          return (
                            <div style={{ padding: '1.25rem', textAlign: 'center', background: 'var(--color-bg-light)', border: '1px dashed var(--color-border)', borderRadius: '10px', color: 'var(--color-text-light)', fontSize: '0.78rem' }}>
                              {t('Chưa có nhật ký/ghi chú nào cho ngày này.')}
                            </div>
                          );
                        }
                        const pageSize = 5;
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto', maxHeight: '400px', paddingRight: '4px' }}>
                      {(() => {
                        ...
                        if (dayTasks.length === 0) {
                          return (
                            <div style={{ padding: '1.25rem', textAlign: 'center', background: 'var(--color-bg-light)', border: '1px dashed var(--color-border)', borderRadius: '10px', color: 'var(--color-text-light)', fontSize: '0.78rem' }}>
                              {t('Chưa có công việc hay lịch hẹn nào cho ngày này.')}
                            </div>
                          );
                        }
                        const pageSize = 5;
