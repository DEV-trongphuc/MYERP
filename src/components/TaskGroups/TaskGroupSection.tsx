...
interface TaskGroupSectionProps {
  ...
  onCreateGroup: (data: { name: string; color: string; icon: string }) => Promise<void>;
  onUpdateGroup: (id: number, data: { name: string; color: string; icon: string }) => Promise<void>;
  onDeleteGroup: (id: number) => Promise<void>;
  onTogglePinGroup?: (id: number) => Promise<void>;
  onReorderGroups?: (orderIds: number[]) => Promise<void>;
  onDropTaskOnGroup?: (taskId: number, targetGroupId: number | null) => void;
  draggedTaskId?: number | null;
  isLightText?: boolean;
  theme?: string;
...
export const TaskGroupSection: React.FC<TaskGroupSectionProps> = ({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Immediately close modal and reset form for instant responsiveness
    closeModal();
  ...
  return (
    <div style={{ marginBottom: '1.25rem', width: '100%', boxSizing: 'border-box' }}>
      {isFilteredGroup ? (
        /* Collapsed View: Khi bấm vào 1 nhóm là khép UI nhóm lại, chỉ hiện title đang xem nhóm và nút <- Quay lại */
        <div style={{
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'space-between',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          {/* Style tag for translucent placeholder */}
          {isLightText && (
            <style>{`
      ...
      {/* Horizontal Scrollable Carousel of Pure White Droppable Group Cards */}
      <div style={{
        display: 'flex',
        alignItems: 'stretch',
        gap: '10px',
        ...
        {/* 1. Card "Tất cả công việc" - White Card */}
        <DroppableGroupCard
          id="group-all"
          groupId="all"
        ...
        {/* 2. Card "Công việc cá nhân" - System Default Folder */}
        <DroppableGroupCard
          id="group-personal"
          groupId="personal"
        ...
        {/* 2. Custom Group Cards - Pure White Droppable & Reorderable Cards */}
        ...
        {/* 3. Card "Chưa phân nhóm" - Luôn ở cuối cùng */}
        <DroppableGroupCard
          id="group-unassigned"
          groupId="unassigned"
      ...
      {/* Modal Tạo / Sửa nhóm - RENDERED TO BODY VIA PORTAL WITH SMOOTH ANIMATION */}
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder={t('Ví dụ: Dự án tuyển sinh, Việc gấp, Hành chính...')}
                  value={groupName}
              ...
              {/* Preview Card - NO LEFT THICK BORDER (bỏ viền màu trái) */}
              <div style={{
                padding: '12px 14px',
                borderRadius: '10px',
                background: '#f8fafc',
      ...
      {/* Modal Xác nhận xóa nhóm - RENDERED TO BODY VIA PORTAL WITH SMOOTH ANIMATION */}
      {typeof document !== 'undefined' && createPortal(
        <AnimatePresence>
          {deletingId !== null && (
            <motion.div 
    ...
    <div style={{ position: 'relative', display: 'inline-block' }} onClick={e => e.stopPropagation()}>
      <button
        ...
        style={{
          ...
          border: groupName ? `1px solid ${groupColor}40` : (isLightText ? '1px dashed rgba(255,255,255,0.3)' : '1px dashed var(--color-border)'),
          color: groupName ? groupColor : (isLightText ? '#cbd5e1' : 'var(--color-text-muted)'),
          fontSize: '0.675rem',
          fontWeight: 700,
          cursor: 'pointer',
          transition: 'border-color 0.15s ease'
