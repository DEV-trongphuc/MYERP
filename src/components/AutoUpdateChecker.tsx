...
export const AutoUpdateChecker: React.FC = () => {
  ...
  const triggerUpdate = (serverVer: string) => {
    ...
    try {
      const reloadKey = `myerp_reload_${serverVer}`;
      ...
      const snoozeUntil = sessionStorage.getItem('myerp_update_snooze_until');
    ...
    if (document.hidden) {
      try {
        localStorage.setItem(`myerp_reload_${serverVer}`, String(Date.now()));
      } catch {}
  ...
  const handleSnooze = (durationMinutes = 60) => {
    ...
    try {
      sessionStorage.setItem('myerp_update_snooze_until', String(Date.now() + durationMinutes * 60 * 1000));
    } catch {}
  };
    ...
    >
      <div
        style={{
          ...
          boxShadow: '0 20px 40px -8px rgba(0, 0, 0, 0.16), 0 4px 16px rgba(0, 0, 0, 0.06)',
