...
export const WorkspaceCustomizerModal: React.FC<WorkspaceCustomizerModalProps> = ({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Check minimum dimensions (1280x720) to prevent blurriness
    ...
    img.onload = async () => {
      ...
      try {
        ...
        if (res.data && res.data.success && res.data.file_url) {
          ...
          // Fallback: Use FileReader data url if upload endpoint has issues
    ...
    >
      ...
      >
        ...
        <div style={{ padding: '20px 24px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '22px' }}>
          ...
          {/* 1. Bố cục Card (Columns Selector) */}
          <div>
          ...
          {/* 2. Chọn Hình Nền (Wallpaper & Patterns) */}
          <div>
            ...
            {/* Tab 3: Upload Custom Background */}
            {activeTab === 'custom' && (
          ...
          {/* 3. Lớp phủ làm dịu nền (Overlay Opacity Slider) */}
          {selectedBg && (
            <div style={{
              background: 'var(--color-bg)',
              padding: '14px 16px',
