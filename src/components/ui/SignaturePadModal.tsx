...
export const SignaturePadModal: React.FC<SignaturePadModalProps> = ({
  ...
  const { t } = useLanguage();
  const isSigningMode = !!title;
  const [signMethod, setSignMethod] = useState<'saved' | 'draw'>(initialSignatureUrl ? 'saved' : 'draw');
  ...
  useEffect(() => {
    if (isOpen) {
      setUploadedImg(initialSignatureUrl || null);
      setSignMethod(initialSignatureUrl ? 'saved' : 'draw');
      setHasDrawn(false);
    }
  }, [isOpen, initialSignatureUrl]);
  ...
  useEffect(() => {
    if (isOpen && (isSigningMode ? signMethod === 'draw' : activeTab === 'draw') && canvasRef.current) {
  ...
  }, [isOpen, activeTab, signMethod, isSigningMode, penColor, strokeWidth]);
  ...
  const handleSaveSignature = async () => {
    ...
    if (isSigningMode) {
      if (signMethod === 'saved') {
        if (!initialSignatureUrl) {
          toast.error(t('Không tìm thấy chữ ký mẫu đã lưu'));
          return;
        }
        finalSignatureUrl = initialSignatureUrl;
      } else {
        const canvas = canvasRef.current;
        if (!canvas || !hasDrawn) {
          toast.error(t('Vui lòng vẽ chữ ký trước khi xác nhận'));
          return;
        }
    ...
    try {
      ...
      toast.success(isSigningMode ? t('Đã ký xác nhận thành công') : t('Đã lưu cấu hình chữ ký thành công'));
      onClose();
    } catch (err: any) {
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800, color: 'var(--color-text)' }}>
              {title || t('Thiết lập Chữ ký Mẫu Cá nhân')}
            </h3>
            <p style={{ margin: '4px 0 0', fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>
              {subtitle || t('Chữ ký này sẽ được sử dụng để ký nhanh chứng từ và phiếu hợp tác.')}
            </p>
          </div>
          <button
            type="button"
            ...
            <div style={{ display: 'flex', gap: '8px', borderBottom: '1px solid var(--color-border)', paddingBottom: '8px' }}>
              <button
                ...
                style={{
                  ...
                  borderRadius: '8px',
                  border: 'none',
                  background: signMethod === 'saved' ? 'rgba(189, 29, 45, 0.1)' : 'transparent',
                  color: signMethod === 'saved' ? '#BD1D2D' : 'var(--color-text-muted)',
                  fontWeight: signMethod === 'saved' ? 700 : 500,
                  fontSize: '0.85rem',
                  cursor: 'pointer',
                  display: 'flex',
                  alignItems: 'center',
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: '8px',
                  border: 'none',
                  background: signMethod === 'draw' ? 'rgba(189, 29, 45, 0.1)' : 'transparent',
                  color: signMethod === 'draw' ? '#BD1D2D' : 'var(--color-text-muted)',
                  fontWeight: signMethod === 'draw' ? 700 : 500,
                  fontSize: '0.85rem',
                  cursor: 'pointer',
                  display: 'flex',
                  alignItems: 'center',
        ...
        {isSigningMode ? (
          ...
          signMethod === 'saved' && initialSignatureUrl ? (
            /* Sub-tab 1: Use Saved Signature */
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{
                ...
                <img 
                  src={initialSignatureUrl.startsWith('http') || initialSignatureUrl.startsWith('data:') ? initialSignatureUrl : `https://myerp.ideas.edu.vn/backend/${initialSignatureUrl.replace(/^\/+/, '')}`} 
                  alt="Chữ ký mẫu đã lưu" 
                  style={{ maxHeight: '160px', objectFit: 'contain' }} 
                />
              </div>
              <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--color-success)', fontWeight: 650, textAlign: 'center' }}>
                {t('✓ Hệ thống sẽ sử dụng chữ ký mẫu đã thiết lập trong hồ sơ cá nhân của bạn.')}
              </p>
            </div>
          ) : (
            /* Sub-tab 2: Draw New Signature */
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '8px' }}>
          ...
          <button
            ...
            disabled={saving || (isSigningMode ? (signMethod === 'saved' ? !initialSignatureUrl : !hasDrawn) : (activeTab === 'draw' ? !hasDrawn : !uploadedImg))}
            style={{
              padding: '8px 18px',
              borderRadius: '8px',
              border: 'none',
              ...
              opacity: saving || (isSigningMode ? (signMethod === 'saved' ? !initialSignatureUrl : !hasDrawn) : (activeTab === 'draw' ? !hasDrawn : !uploadedImg)) ? 0.6 : 1,
              display: 'flex',
              alignItems: 'center',
              gap: '6px'
            }}
