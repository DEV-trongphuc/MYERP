...
export function formatCommentBody(body?: string): string {
  ...
  // 1. Transform existing <a class="comment-attachment-chip"...> that contain legacy emojis or lack badge
  html = html.replace(/<a([^>]*?class=["'][^"']*?comment-attachment-chip[^"']*?["'][^>]*?)>([\s\S]*?)<\/a>/gi, (match, attrs, innerContent) => {
    // Extract href / data-file-url / data-file-name
    ...
    // If fileName not in attrs, parse from innerContent
    if (!fileName) {
      // Remove html tags to find filename
  ...
  // 2. Transform legacy plain text or inline patterns with emoji files: [📕|📄|📊|📝|📦|🖼️|📎] filename.ext (size)
