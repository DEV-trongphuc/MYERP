/**
 * taskBodyParser.ts
 * Tiện ích bóc tách JSON an toàn và phục hồi định dạng văn bản cho các công việc trong MYERP.
 */
 ...
 * (đặc biệt là dữ liệu import từ MISA, các đề mục emoji, từ khóa và câu dính sau dấu chấm).
 */
export function formatVietnameseDescription(rawText: string | null | undefined): string {
  ...
  while (text.startsWith('{') && text.endsWith('}')) {
    try {
      ...
      try {
        ...
        // Fallback: nếu chứa "description", bóc tách bằng regex
  ...
  // Loại bỏ các cờ và mảnh JSON thô còn sót lại (kể cả phần đuôi JSON dở dang từ sync)
  ...
  // Dọn dẹp dấu ngoặc nhọn hoặc phẩy mồ côi ở đầu và cuối chuỗi
  ...
  // Ví dụ: "Ví dụ:Giống như" -> "Ví dụ:\nGiống như" hoặc "Lưu ý:Không phải" -> "Lưu ý:\nKhông phải"
  ...
  // 5. Tách ngày tháng năm dính liền chữ hoa: ví dụ "23/09/2026CHUYẾN ĐI" -> "23/09/2026\nCHUYẾN ĐI"
  ...
  // Chú ý tránh các từ viết tắt phổ biến: TP.HCM, ThS., TS., Dr., Mr., Mrs., Ms., vs., etc.
  text = text.replace(/([^0-9A-Za-zÀ-ỸĐà-ỹđ]|^|[a-zà-ỹđ]{2,})([.!?])([A-ZÀ-ỸĐ])/gu, (match, prefix, punc, nextChar) => {
    // Không tách nếu là từ viết tắt như TP.HCM
  ...
  // 7. Thêm khoảng trắng sau dấu phẩy nếu dính liền chữ: ví dụ "chị Nữ,Học viên" -> "chị Nữ, Học viên"
...
export function parseTaskBody(rawBody: string | null | undefined): ParsedTaskBody {
  ...
  // Lặp unnest nếu bị JSON.stringify nhiều lần
    ...
    try {
      ...
      if (p && typeof p === 'object') {
        ...
        if (p.erp_task && typeof p.erp_task === 'object') {
          // Gộp các thuộc tính trong erp_task
          Object.assign(result.rawMeta, p.erp_task);
        }
        // Gộp các thuộc tính ở root
...
export function extractCleanCardDescription(rawBody: string | null | undefined): string {
  ...
  // Bỏ thẻ HTML để hiển thị dạng plain text trên thẻ
  desc = desc.replace(/<[^>]*>/g, ' ');
  // Bỏ các ký tự đặc biệt khoảng trắng thừa
  desc = desc.replace(/\s+/g, ' ').trim();

  // Kiểm tra lần cuối: nếu còn sót chuỗi JSON do bất kỳ lý do gì, lọc sạch triệt để
 ...
 * 1. Có nhãn/scope cá nhân: personal_task, ca_nhan, personal, scope='personal'
 * 2. HOẶC là công việc mà CHỈ CÓ 1 MÌNH người đó tham gia:
 *    - Thuộc về user (người thực hiện hoặc người tạo)
 *    - Không có người liên quan khác (participant_ids)
 *    - Không có người duyệt khác (approver_id)
 *    - Không có checklist item nào giao cho người khác
 *    - Không gắn với dự án chung, chiến dịch chung hay khách hàng/deal chung
 */
export function isTaskPersonalForUser(task: any, currentUserId: number | string | null | undefined): boolean {
  ...
  // Phải thuộc về user này (hoặc được giao hoặc do user tự tạo)
  if (taskUid !== uid && taskCreatedBy !== uid) {
    return false;
  }

  // 1. Nhận diện theo nhãn/scope cá nhân rõ ràng
  ...
  // 2. Kiểm tra nếu có người khác cùng tham gia:
  // a) Người liên quan khác
  ...
  // c) Giao việc con (subtask) cho người khác trong checklist
  ...
  // d) Gắn với khách hàng/deal/công ty hay dự án nhóm
  ...
  // Nếu thỏa mãn tất cả: chỉ 1 mình user thực hiện và tạo, không liên quan ai khác -> Tự động là việc cá nhân!
  return true;
}
