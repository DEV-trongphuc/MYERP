                ...
                if ($nowTime >= $reminderStartTimestamp && $nowTime < $dueTimestamp) {
                    ...
                    foreach ($studentsList as $student) {
                        ...
                        if (!$alreadySent) {
                            ...
                            if (!empty($studEmail)) {
                                try {
                                    sendEmailNotification($studEmail, "[IDEAS] Nhắc nhở hạn nộp luận văn: {$ms['milestone']}", $msgTitle, nl2br($msgBody), '', false, 0, true);
                    ...
                    if ($nowTime >= $reminderStartTimestamp && $nowTime < $sessionTimestamp) {
                        ...
                        foreach ($studentsList as $student) {
                            ...
                            if (!$alreadySent) {
                                ...
                                try {
                                    sendEmailNotification($studEmail, "[IDEAS] Nhắc nhở lịch học chính thức - Môn {$sub['name']}", $msgTitle, nl2br($msgBody), '', false, 0, true);
                    ...
                    if ($nowTime >= $reminderStartTimestamp && $nowTime < $semTimestamp) {
                        ...
                        foreach ($studentsList as $student) {
                            ...
                            if (!$alreadySent) {
                                ...
                                try {
                                    sendEmailNotification($studEmail, "[IDEAS] Nhắc nhở lịch học chuyên đề - Môn {$sub['name']}", $msgTitle, nl2br($msgBody), '', false, 0, true);
                    ...
                    if ($nowTime >= $reminderStartTimestamp && $nowTime < $dueTimestamp) {
                        ...
                        foreach ($studentsList as $student) {
                            ...
                            if (!$alreadySent) {
                                ...
                                try {
                                    sendEmailNotification($studEmail, "[IDEAS] Nhắc nhở hạn nộp bài tập: {$asn['name']}", $msgTitle, nl2br($msgBody), '', false, 0, true);
                ...
                if ($nowTime >= $reminderStartTimestamp && $nowTime < $sessTimestamp) {
                    ...
                    foreach ($studentsList as $student) {
                        ...
                        if (!$alreadySent) {
                            ...
                            try {
                                sendEmailNotification($studEmail, "[IDEAS] Lớp học sắp bắt đầu - {$sess['title']}", $msgTitle, nl2br($msgBody), '', false, 0, true);
