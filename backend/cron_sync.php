...
function checkTaskDueSlaAlerts($conn) {
    ...
    // 1. Get global work start time (default 08:00)
    $resSettings = $conn->query("SELECT setting_value FROM system_settings WHERE setting_key = 'global_work_start_time' LIMIT 1");
    $globalWorkStart = '08:00';
    if ($resSettings && $rowS = $resSettings->fetch_assoc()) {
        if (!empty($rowS['setting_value'])) {
            $globalWorkStart = substr($rowS['setting_value'], 0, 5);
        }
    }
    ...
    // Rule: Chờ đến giờ vào ca hôm sau (work shift start time, vd 08:00) mới gửi cảnh báo quá hạn
    if ($currentTime < $globalWorkStart) {
        logSync("checkTaskDueSlaAlerts skipped: Current time ($currentTime) is before shift start time ($globalWorkStart).");
        return;
    }
    
    // 2. Query tasks:
    // Nếu due_date chỉ có ngày (00:00:00) hoặc 23:59:59, hạn thực tế là hết ngày đó (23:59:59).
    // Do đó nếu DATE(due_date) = hôm nay thì chưa quá hạn, chỉ quá hạn khi DATE(due_date) < hôm nay.
    // Nếu due_date có giờ cụ thể trong ngày (ví dụ 14:00), thì quá hạn khi due_date <= NOW().
...
function checkSubtaskDueAlerts($conn) {
    ...
    $currentTime = date('H:i');
    if ($currentTime < $globalWorkStart) {
        logSync("checkSubtaskDueAlerts skipped: Current time ($currentTime) is before shift start time ($globalWorkStart).");
        return;
    }
        ...
        while ($row = $res->fetch_assoc()) {
            ...
            foreach ($checklist as &$item) {
                if (empty($item['done']) && !empty($item['due_date'])) {
                    ...
                    $isEndOfDay = (date('H:i:s', $dueTime) === '23:59:59' || date('H:i:s', $dueTime) === '00:00:00' || date('H:i', $dueTime) === '23:59');
                    
                    // If date-only/end-of-day, overdue only when date < today
                    $isOverdue = false;
                    if ($isEndOfDay) {
                        $isOverdue = (date('Y-m-d', $dueTime) < date('Y-m-d'));
                    } else {
                        $isOverdue = ($dueTime !== false && $dueTime <= time());
                    }
...
/**
 * Helper to check if today is a scheduled working day for user
 */
function isUserWorkingDayForReminders($workSchedule, $holidaySchedulesJson = '[]') {
    $currDate = date('Y-m-d');
    $dayOfWeek = (int)date('N'); // 1 (Mon) - 7 (Sun)
    
    // Check holiday
    if (!empty($holidaySchedulesJson)) {
        $holidays = json_decode($holidaySchedulesJson, true);
        if (is_array($holidays)) {
            foreach ($holidays as $h) {
                ...
                if (!empty($hStart) && !empty($hEnd)) {
                    if ($currDate >= $hStart && $currDate <= $hEnd) {
    ...
    if (empty($workSchedule) || $workSchedule === 'monday_to_friday') {
        return $dayOfWeek >= 1 && $dayOfWeek <= 5;
    }
    if ($workSchedule === 'monday_to_saturday' || $workSchedule === 'monday_to_noon_saturday') {
        return $dayOfWeek >= 1 && $dayOfWeek <= 6;
    }
    if ($workSchedule === 'all_week') {
        return true;
    }
    $decoded = json_decode($workSchedule, true);
    if (is_array($decoded)) {
        return in_array($dayOfWeek, $decoded, false) || in_array((string)$dayOfWeek, $decoded, true);
    }
    return $dayOfWeek >= 1 && $dayOfWeek <= 5;
}

/**
 * Ca sáng: Nhắc nhở nhân viên CHƯA chấm công vào khi trễ 15 phút so với giờ bắt đầu ca (chung hoặc cá nhân)
 */
function sendCheckInMissingReminders($conn) {
    $todayStr = date('Y-m-d');
    $nowStr = date('H:i');

    // 1. Load global settings
    $resSettings = $conn->query("SELECT setting_key, setting_value FROM system_settings WHERE setting_key IN ('global_work_start_time', 'global_work_schedule', 'holiday_schedules')");
    $settings = [];
    if ($resSettings) {
    ...
    $globalWorkStart = !empty($settings['global_work_start_time']) ? substr($settings['global_work_start_time'], 0, 5) : '08:00';
    $globalWorkSchedule = $settings['global_work_schedule'] ?? 'monday_to_friday';
    $holidaysJson = $settings['holiday_schedules'] ?? '[]';
    ...
    while ($user = $res->fetch_assoc()) {
        ...
        // Check user working schedule
        $userSchedule = ($user['use_custom_work_hours'] == 1 && !empty($user['work_schedule'])) ? $user['work_schedule'] : $globalWorkSchedule;
        if (!isUserWorkingDayForReminders($userSchedule, $holidaysJson)) continue;

        // Effective work start time
        $workStart = ($user['use_custom_work_hours'] == 1 && !empty($user['work_start_time'])) ? substr($user['work_start_time'], 0, 5) : $globalWorkStart;

        // Calculate trigger time: exactly 15 minutes after work start
        $triggerTimestamp = strtotime("$todayStr $workStart") + (15 * 60);
        $triggerTime = date('H:i', $triggerTimestamp);

        // If current time hasn't reached 15 minutes late yet, skip
        if ($nowStr < $triggerTime) continue;

        // Check if user has already checked in today
        $stmtCheck = $conn->prepare("SELECT id FROM check_ins WHERE user_id = ? AND check_in_date = ? AND check_in_time IS NOT NULL LIMIT 1");
        $stmtCheck->bind_param("is", $userId, $todayStr);
        $stmtCheck->execute();
        $hasCheckedIn = (bool)$stmtCheck->get_result()->fetch_assoc();
        $stmtCheck->close();
        if ($hasCheckedIn) continue;

        // Check if notification already sent today
        $chk = $conn->prepare("SELECT id FROM sent_notifications WHERE user_id = ? AND notify_type = 'checkin_missing_reminder' AND notify_date = ? LIMIT 1");
        $chk->bind_param("is", $userId, $todayStr);
        $chk->execute();
        $hasSent = (bool)$chk->get_result()->fetch_assoc();
        $chk->close();
        ...
        // Dispatch reminder via NotificationService (In-App Bell, Zalo, Telegram, Email)
        NotificationService::send($pdo, $tenantId, 'CHECKIN_MISSING_REMINDER', [
            'recipients' => [$user],
            'user_name' => $user['full_name'],
            'work_start' => $workStart
        ]);

        $ins = $conn->prepare("INSERT IGNORE INTO sent_notifications (user_id, notify_type, notify_date) VALUES (?, 'checkin_missing_reminder', ?)");
        $ins->bind_param("is", $userId, $todayStr);
        $ins->execute();
        $ins->close();

        logSync("Sent Morning Check-In missing reminder (15m late) to: {$user['full_name']} (User ID: {$userId}, Shift Start: {$workStart}, Trigger: {$triggerTime})");
    }
}

/**
 * Ca chiều: Nhắc nhở nhân viên CHƯA chấm công ra khi trễ 15 phút so với giờ kết thúc ca (chung hoặc cá nhân)
 */
function sendCheckOutMissingReminders($conn) {
    $todayStr = date('Y-m-d');
    $nowStr = date('H:i');

    // 1. Load global settings
    $resSettings = $conn->query("SELECT setting_key, setting_value FROM system_settings WHERE setting_key IN ('global_work_end_time', 'require_checkout')");
    $settings = [];
    if ($resSettings) {
        while ($r = $resSettings->fetch_assoc()) {
            $settings[$r['setting_key']] = $r['setting_value'];
    ...
    $sql = "
        SELECT u.id, u.tenant_id, u.full_name, u.email, u.zalo_chat_id, u.telegram_chat_id, u.role,
               u.use_custom_work_hours, u.work_end_time,
               c.check_in_time
        FROM check_ins c
        JOIN users u ON c.user_id = u.id
    ...
    while ($user = $res->fetch_assoc()) {
        ...
        // Calculate trigger time: exactly 15 minutes after work end
        $triggerTimestamp = strtotime("$todayStr $workEnd") + (15 * 60);
        $triggerTime = date('H:i', $triggerTimestamp);

        // If current time hasn't reached 15 minutes past work end yet, skip
        if ($nowStr < $triggerTime) continue;

        // Check if notification already sent today
        $chk = $conn->prepare("SELECT id FROM sent_notifications WHERE user_id = ? AND notify_type = 'checkout_missing_reminder' AND notify_date = ? LIMIT 1");
        $chk->bind_param("is", $userId, $todayStr);
        $chk->execute();
        $hasSent = (bool)$chk->get_result()->fetch_assoc();
        $chk->close();
        ...
        // Dispatch reminder via NotificationService (In-App Bell, Zalo, Telegram, Email)
        NotificationService::send($pdo, $tenantId, 'CHECKOUT_MISSING_REMINDER', [
            'recipients' => [$user],
            'user_name' => $user['full_name'],
            'work_end' => $workEnd
        ]);

        $ins = $conn->prepare("INSERT IGNORE INTO sent_notifications (user_id, notify_type, notify_date) VALUES (?, 'checkout_missing_reminder', ?)");
        $ins->bind_param("is", $userId, $todayStr);
        $ins->execute();
        $ins->close();

        logSync("Sent Afternoon Check-Out missing reminder (15m late) to: {$user['full_name']} (User ID: {$userId}, Shift End: {$workEnd}, Trigger: {$triggerTime})");
    }
    $stmt->close();
}
...
/**
 * 20:00 tối ngày cuối cùng của kỳ nghỉ lễ kéo dài > 2 ngày:
 * Tự động gửi thông báo nhắc nhở ngày mai bắt đầu làm việc lại cho toàn thể nhân viên.
 */
function sendHolidayReturnReminders($conn) {
    $todayStr = date('Y-m-d');
    $nowStr = date('H:i');

    // Chỉ kích hoạt từ 20:00 (8h tối) trở đi
    if ($nowStr < '20:00') {
        return;
    }

    $resSettings = $conn->query("SELECT setting_key, setting_value FROM system_settings WHERE setting_key IN ('global_work_start_time', 'holiday_schedules')");
    $settings = [];
    if ($resSettings) {
        while ($r = $resSettings->fetch_assoc()) {
            $settings[$r['setting_key']] = $r['setting_value'];
    ...
    foreach ($holidays as $h) {
        $hStart = $h['start'] ?? $h['start_date'] ?? $h['date'] ?? '';
        $hEnd = $h['end'] ?? $h['end_date'] ?? $h['date'] ?? '';
        $hName = $h['name'] ?? 'Nghỉ lễ';
        $hId = $h['id'] ?? md5($hStart . '_' . $hEnd);

        if (empty($hStart) || empty($hEnd)) continue;

        $startTs = strtotime($hStart);
        $endTs = strtotime($hEnd);
        $totalDays = (int)round(($endTs - $startTs) / 86400) + 1;

        // Chỉ áp dụng cho kỳ nghỉ dài hơn 2 ngày (> 2 ngày)
        if ($totalDays <= 2) continue;

        // Hôm nay phải là ngày cuối cùng của kỳ nghỉ (ngày trước khi vào làm lại)
        if ($todayStr !== $hEnd) continue;

        $nextDayTs = $endTs + 86400;
        $dayOfWeekNum = (int)date('N', $nextDayTs);
        $dayOfWeekNames = [
            1 => 'Thứ Hai',
            2 => 'Thứ Ba',
            3 => 'Thứ Tư',
        ...
        $nextDayText = ($dayOfWeekNames[$dayOfWeekNum] ?? 'Thứ ' . $dayOfWeekNum) . ', ngày ' . date('d/m/Y', $nextDayTs);

        // Kiểm tra xem đã gửi thông báo hôm nay chưa
        $notifyType = 'holiday_return_reminder_' . $hId;
        $chk = $conn->prepare("SELECT id FROM sent_notifications WHERE user_id = 0 AND notify_type = ? AND notify_date = ? LIMIT 1");
        $chk->bind_param("ss", $notifyType, $todayStr);
        $chk->execute();
        $hasSent = (bool)$chk->get_result()->fetch_assoc();
        $chk->close();
        if ($hasSent) continue;
        ...
        NotificationService::send($pdo, 1, 'HOLIDAY_RETURN_REMINDER', [
            'holiday_name' => $hName,
            'next_day_text' => $nextDayText,
            'work_start' => $globalWorkStart
        ]);

        $ins = $conn->prepare("INSERT IGNORE INTO sent_notifications (user_id, notify_type, notify_date) VALUES (0, ?, ?)");
        $ins->bind_param("ss", $notifyType, $todayStr);
        $ins->execute();
        $ins->close();

        logSync("Sent Holiday Return Reminder at 20:00 for holiday '$hName' (Resume on: $nextDayText, Work start: $globalWorkStart)");
    }
}
...
if (!defined('DIAG_TOKEN')) {
    ...
    // --- Chạy kiểm tra nhắc nhở trễ 15 phút ca sáng (Chưa chấm công Vào ca) ---
    try {
        if (function_exists('sendCheckInMissingReminders')) {
            sendCheckInMissingReminders($conn);
        }
    } catch (Exception $e) {
        logSync("Error running sendCheckInMissingReminders: " . $e->getMessage());
    }

    // --- Chạy kiểm tra nhắc nhở trễ 15 phút ca chiều (Chưa chấm công Ra ca) ---
    try {
        if (function_exists('sendCheckOutMissingReminders')) {
            sendCheckOutMissingReminders($conn);
        }
    } catch (Exception $e) {
        logSync("Error running sendCheckOutMissingReminders: " . $e->getMessage());
    }
    ...
    // --- Chạy nhắc nhở 20h tối trước ngày đi làm lại sau kỳ nghỉ lễ dài (> 2 ngày) ---
    try {
        if (function_exists('sendHolidayReturnReminders')) {
            sendHolidayReturnReminders($conn);
        }
    } catch (Exception $e) {
