...
class NoteController {
    ...
    public function index(array $auth, string $type, int $entityId): void {
        ...
        // Cooperating partners/co-care sales must see all notes and interactions (100% visibility)
    ...
    public function store(array $auth, string $type, int $entityId): void {
        ...
        if (str_starts_with(trim($b['body']), '[Chuyển trạng thái Pipeline]')) {
            ...
            if ($type === 'contact') {
                ...
                $this->db->prepare("UPDATE contacts SET last_contact = NOW(), security_expires_at = ? WHERE id = ?")
        ...
        if ($type === 'contact') {
            ...
            $updateSql = "UPDATE contacts SET last_contact = NOW(), security_expires_at = ?";
    ...
    public function destroy(array $auth, int $id): void {
        ...
        // 1. Verify existence and permission (Skipping legacy attachments field check)
        $check = $this->db->prepare("SELECT entity_type, entity_id, body FROM notes WHERE id=? AND tenant_id=?" . (!in_array($auth['role'], ['admin', 'superadmin', 'super_admin', 'manager', 'director'], true) ? " AND user_id=?" : ""));
